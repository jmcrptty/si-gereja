            ...
            <tbody>
                @foreach ($krisma as $data_krisma)
                    <tr>
                        ...
                        <td>{{ $data_krisma->tanggal_daftar->format('d M Y')}}</td>
                        <td>
                            <div class="flex-wrap gap-2 d-flex justify-content-center">
                                ...
                                {{-- Konfirmasi --}}
                                <button type="button" class="border-0 badge bg-success fs-6" data-bs-toggle="modal" data-bs-target="#confirmModalPenerimaanKrisma{{ $data_krisma->id }}">
                                    <i class="fa-solid fa-check"></i> Konfirmasi Penerimaan
                                </button>

                                {{-- Tolak --}}
                                <button type="button" class="border-0 badge bg-danger fs-6" data-bs-toggle="modal" data-bs-target="#rejectModalLabelPenerimaanKrisma{{ $data_krisma->id }}">
                    ...
                    <!-- Modal Konfirmasi -->
                    <div class="modal fade" id="confirmModalPenerimaanKrisma{{ $data_krisma->id }}" tabindex="-1" aria-labelledby="confirmModalPenerimaanKrismaLabel{{ $data_krisma->id }}" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered">
                            <div class="border-0 shadow modal-content">
                                <div class="pb-0 border-0 modal-header">
                                    <h5 class="modal-title text-danger fw-bold" id="confirmModalPenerimaanKrismaLabel{{ $data_krisma->id }}">
                                ...
                                <div class="pt-0 border-0 modal-footer justify-content-center">
                                    <form action="{{ route('sekretaris.setujuKrisma.penerimaan', $data_krisma->id) }}" method="POST" class="d-inline">
                                        @csrf
                    ...
                    <div class="modal fade" id="rejectModalLabelPenerimaanKrisma{{ $data_krisma->id }}" tabindex="-1" aria-labelledby="rejectModalLabelPenerimaanKrisma{{ $data_krisma->id }}" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered">
                            <div class="border-0 shadow modal-content">
                                <div class="pb-0 border-0 modal-header">
                                    <h5 class="modal-title text-danger fw-bold" id="rejectModalLabelPenerimaanKrisma{{ $data_krisma->id }}">
                                        <i class="fas fa-exclamation-circle"></i> Konfirmasi Data
                                    </h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                </div>
                                ...
                                <div class="pt-0 border-0 modal-footer justify-content-center">
                                    <form action="{{ route('sekretaris.tolakKrisma.penerimaan', $data_krisma->id) }}" method="POST" class="d-inline">
                                        @csrf
